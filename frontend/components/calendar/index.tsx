...
export const monthsForLocale = (
  ...
  return [...new Array(MONTHS_IN_YEAR).keys()].map(m =>
    format(new Date(Date.UTC(BASE_YEAR, m, BASE_DAY)))
  )
}

export const daysForLocale = (locale: string, startDay: number) => {
  const weekdays: string[] = []
  const baseDate = new Date(BASE_YEAR_DAYS, BASE_MONTH, startDay)
      ...
      <PopoverContent className="w-40 p-0">
        <Command
          filter={(filterValue, search) => {
            const label = data.find(item => item.value === filterValue)?.label
...
export const CalendarBody = ({ features, children }: CalendarBodyProps) => {
  ...
  const prevMonthData = useMemo(() => {
    ...
    const prevMonthYear = month === FIRST_MONTH ? year - 1 : year
  ...
  const nextMonthData = useMemo(() => {
    ...
    const nextMonthYear = month === LAST_MONTH ? year + 1 : year
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    days.push(
      ...
      >
        ...
        {featuresForDay.length > MAX_FEATURES_DISPLAY && (
          <span className="block text-muted-foreground text-xs">
            +{featuresForDay.length - MAX_FEATURES_DISPLAY} more
          </span>
        )}
      </div>
    )
      ...
      {days.map((day, dayIndex) => (
        <div
          className={cn(
            ...
            dayIndex % DAYS_IN_WEEK === DAYS_IN_WEEK - 1 && 'border-r-0'
          )}
...
export const CalendarMonthPicker = ({
  ...
  const monthData = useMemo(() => {
    return monthsForLocale(locale).map((monthLabel, monthIndex) => ({
